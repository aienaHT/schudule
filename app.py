...
from models import db, User, Teacher, Subject, Group, Room, TeacherSubject, AppSettings, GroupSubject, ScheduleEntry, \
    MainScheduleEntry, AutoFillLog, GroupPractice
...
def create_app():
    ...
    def api_add_schedule():
        ...
        try:
            ...
            if is_combined:
                neighbor_lesson = lesson_number + 1 if lesson_number % 2 == 1 else lesson_number - 1

                if is_main:
                    neighbor = MainScheduleEntry.query.filter_by(
                        group_id=group.id,
                        day=day,
                        lesson_number=neighbor_lesson,
                        semester=int(data.get('semester', 1))
                    ).first()
                else:
                    neighbor = ScheduleEntry.query.filter_by(
                        group_id=group.id,
                        day=day,
                        lesson_number=neighbor_lesson,
                ...
                if not neighbor or neighbor.subject_id != subject.id:
                    return jsonify({'success': False,
                                    'message': 'Совмещенное занятие должно состоять из двух уроков одного предмета подряд'})
    ...
    # ИСПРАВЛЕННЫЙ маршрут для удаления из основного расписания
    ...
    # ИСПРАВЛЕННЫЙ маршрут для удаления из текущего расписания
    ...
    def api_update_schedule(id):
        ...
        try:
            ...
            if 'is_combined' in data:
                entry.is_combined = data['is_combined']
        ...
        try:
            ...
            for gs in subjects:
                result.append({
                    ...
                    'subject_name': gs.subject.name if gs.subject else 'Неизвестно',
    ...
    # ========== API ДЛЯ УПРАВЛЕНИЯ ПРЕДМЕТАМИ И УЧИТЕЛЯМИ ==========

    # Добавление нового предмета
    @app.route('/api/data/subjects/add', methods=['POST'])
    @login_required
    def api_add_subject():
        if current_user.role != 'admin':
            return jsonify({'success': False, 'message': 'Доступ запрещен'})
        ...
        try:
            ...
            if not name:
                return jsonify({'success': False, 'message': 'Название предмета обязательно'})
            ...
            return jsonify({'success': True, 'message': 'Предмет добавлен', 'id': subject.id, 'name': subject.name})
    ...
    @app.route('/api/data/subjects/<int:id>/update', methods=['PUT'])
    @login_required
    def api_update_subject(id):
        ...
        try:
            ...
            existing = Subject.query.filter_by(name=name).filter(Subject.id != id).first()
            if existing:
                return jsonify({'success': False, 'message': 'Предмет с таким названием уже существует'})
            ...
            return jsonify({'success': True, 'message': 'Предмет обновлен', 'id': subject.id, 'name': subject.name})
    ...
    # Удаление предмета
    @app.route('/api/data/subjects/<int:id>/delete', methods=['DELETE'])
    @login_required
    def api_delete_subject(id):
        ...
        try:
            subject = Subject.query.get(id)
            if subject:
                # Проверяем, не используется ли предмет в расписании
                schedule_entries = ScheduleEntry.query.filter_by(subject_id=id).count()
                main_schedule_entries = MainScheduleEntry.query.filter_by(subject_id=id).count()
                group_subjects = GroupSubject.query.filter_by(subject_id=id).count()
                practices = GroupPractice.query.filter_by(subject_id=id).count()

                total_uses = schedule_entries + main_schedule_entries + group_subjects + practices

                if total_uses > 0:
                    return jsonify({
                        'success': False,
                        'message': f'Невозможно удалить предмет. Он используется в {total_uses} местах.'
                    })
    ...
    def api_add_teacher():
        ...
        try:
            ...
            if not name:
                return jsonify({'success': False, 'message': 'Имя преподавателя обязательно'})

            existing = Teacher.query.filter_by(name=name).first()
            if existing:
                return jsonify({'success': False, 'message': 'Преподаватель с таким именем уже существует'})
            ...
            return jsonify(
                {'success': True, 'message': 'Преподаватель добавлен', 'id': teacher.id, 'name': teacher.name})
    ...
    @app.route('/api/data/teachers/<int:id>/update', methods=['PUT'])
    @login_required
    def api_update_teacher(id):
        ...
        try:
            ...
            if not teacher:
                return jsonify({'success': False, 'message': 'Преподаватель не найден'})

            # Проверяем, не существует ли уже преподаватель с таким именем
            existing = Teacher.query.filter_by(name=name).filter(Teacher.id != id).first()
            if existing:
                return jsonify({'success': False, 'message': 'Преподаватель с таким именем уже существует'})
            ...
            return jsonify(
                {'success': True, 'message': 'Преподаватель обновлен', 'id': teacher.id, 'name': teacher.name})
    ...
    @app.route('/api/data/teachers/<int:id>/delete', methods=['DELETE'])
    @login_required
    def api_delete_teacher(id):
        ...
        try:
            teacher = Teacher.query.get(id)
            if teacher:
                # Проверяем, не используется ли преподаватель в расписании
                schedule_entries = ScheduleEntry.query.filter_by(teacher_id=id).count()
                main_schedule_entries = MainScheduleEntry.query.filter_by(teacher_id=id).count()
                group_subjects = GroupSubject.query.filter_by(teacher_id=id).count()
                practices = GroupPractice.query.filter_by(teacher_id=id).count()
                ...
                if total_uses > 0:
                    return jsonify({
                        'success': False,
                        'message': f'Невозможно удалить преподавателя. Он используется в {total_uses} местах.'
                    })

                db.session.delete(teacher)
                db.session.commit()
                return jsonify({'success': True, 'message': 'Преподаватель удален'})
            return jsonify({'success': False, 'message': 'Преподаватель не найден'})
        except Exception as e:
            db.session.rollback()
            return jsonify({'success': False, 'message': str(e)})

    # Получение всех предметов с детальной информацией
    @app.route('/api/data/subjects/detailed')
    @login_required
    def api_get_subjects_detailed():
        if current_user.role != 'admin':
            return jsonify({'success': False, 'message': 'Доступ запрещен'})
        ...
        try:
            ...
            for subject in subjects:
                # Считаем использование предмета
                group_subjects_count = GroupSubject.query.filter_by(subject_id=subject.id).count()
                schedule_count = ScheduleEntry.query.filter_by(subject_id=subject.id).count()
                main_schedule_count = MainScheduleEntry.query.filter_by(subject_id=subject.id).count()
                practice_count = GroupPractice.query.filter_by(subject_id=subject.id).count()

                result.append({
                    'id': subject.id,
                    'name': subject.name,
                    'usage_count': group_subjects_count + schedule_count + main_schedule_count + practice_count,
                    'details': {
                        'group_subjects': group_subjects_count,
                        'schedule_entries': schedule_count,
                        'main_schedule_entries': main_schedule_count,
    ...
    # Получение всех преподавателей с детальной информацией
    @app.route('/api/data/teachers/detailed')
    @login_required
    def api_get_teachers_detailed():
        if current_user.role != 'admin':
            return jsonify({'success': False, 'message': 'Доступ запрещен'})
        ...
        try:
            ...
            for teacher in teachers:
                # Считаем нагрузку преподавателя
                group_subjects_count = GroupSubject.query.filter_by(teacher_id=teacher.id).count()
                schedule_count = ScheduleEntry.query.filter_by(teacher_id=teacher.id).count()
                main_schedule_count = MainScheduleEntry.query.filter_by(teacher_id=teacher.id).count()
                practice_count = GroupPractice.query.filter_by(teacher_id=teacher.id).count()

                # Считаем общее количество часов в неделю
                group_subjects = GroupSubject.query.filter_by(teacher_id=teacher.id).all()
                total_hours_per_week = sum(gs.hours_per_week for gs in group_subjects)
    ...
    @app.route('/api/schedule/auto_fill', methods=['POST'])
    @login_required
    def api_auto_fill_schedule():
        if current_user.role != 'admin':
            return jsonify({'success': False, 'message': 'Доступ запрещен'})
        ...
        try:
            ...
            fill_type = data.get('fill_type', 'both')

            result = auto_fill_schedule_logic(week, semester, fill_type)

            return jsonify({
                'success': True,
                'message': 'Расписание автозаполнено',
        ...
        for teacher in teachers:
            ...
            for ts in teacher_subjects:
                groups_detail.append({
                    'group_name': ts.group.name if ts.group else 'Неизвестно',
                    'subject_name': ts.subject.name if ts.subject else 'Неизвестно',
    ...
    def api_clear_week():
        ...
        try:
            ...
            db.session.commit()
            return jsonify(
                {'success': True, 'message': f'Неделя {current_week} очищена и заполнена из основного расписания'})
...
def get_week_parity(week_number):
...
def update_current_schedule_from_main(week_number, semester):
    ...
    week_parity = get_week_parity(week_number)
    ...
    for main_entry in main_entries:
        schedule_entry = ScheduleEntry(
            ...
            semester=semester,
            is_changed=False,
            is_combined=main_entry.is_combined
        )
...
def is_lesson_available(group_id, teacher_id, day, lesson, week, semester, check_main=True, check_current=True):
    """Проверяет, свободен ли урок для группы, преподавателя с учетом четности недели"""

    week_parity = get_week_parity(week)

    # Проверяем основное расписание
    if check_main:
        main_conflict = MainScheduleEntry.query.filter(
            and_(
    ...
    if check_current:
        current_conflict = ScheduleEntry.query.filter(
            and_(
...
def find_available_room(day, lesson, week, semester, exclude_room_id=None):
    """Находит свободную аудиторию на указанное время с учетом четности недели"""
    ...
    for room in rooms:
        ...
        # Проверяем основное расписание
        main_conflict = MainScheduleEntry.query.filter(
            MainScheduleEntry.room_id == room.id,
            MainScheduleEntry.day == day,
            MainScheduleEntry.lesson_number == lesson,
            MainScheduleEntry.semester == semester,
            or_(
        ...
        # Проверяем текущее расписание
        current_conflict = ScheduleEntry.query.filter_by(
            room_id=room.id,
            day=day,
...
def get_day_load(group_id, day, week, semester):
    """Возвращает количество пар у группы в конкретный день"""
    entries = ScheduleEntry.query.filter_by(
        group_id=group_id,
        day=day,
        week_number=week,
    ...
    # Считаем уникальные пары (уроки 1-2 это одна пара)
    pairs = set()
    for entry in entries:
        pair = get_pair_number_by_lesson(day, entry.lesson_number)
        pairs.add(pair)

    return len(pairs)


def auto_fill_schedule_logic(week, semester, fill_type='both'):
    """ИСПРАВЛЕННАЯ функция автозаполнения расписания без дырок"""
    ...
    try:
        ...
        if fill_type in ['current', 'both']:
            ScheduleEntry.query.filter_by(week_number=week, semester=semester).delete()

        if fill_type in ['main', 'both']:
            MainScheduleEntry.query.filter_by(semester=semester).delete()

        # Получаем все группы и дни
        groups = Group.query.order_by(Group.course, Group.name).all()
        days = ["Понедельник", "Вторник", "Среда", "Четверг", "Пятница", "Суббота"]

        # Словарь для отслеживания уже поставленных пар
        placed_pairs = {}  # (group_id, day, pair) -> subject_id

        # Сначала обрабатываем практику для всех групп
        for group in groups:
            practice = GroupPractice.query.filter_by(group_id=group.id).first()
            if practice and practice.day in days:
                if not practice.teacher_id:
                    log_entry.errors += 1
                    print(f"Ошибка: у практики группы {group.name} не назначен преподаватель")
                    continue

                max_pairs = 4 if practice.day != "Суббота" else 2
                placed = False

                # Ищем свободную пару для практики
                pairs_order = list(range(1, max_pairs + 1))
                random.shuffle(pairs_order)

                for pair in pairs_order:
                    if placed:
                        break

                    if (group.id, practice.day, pair) in placed_pairs:
                        continue

                    lessons = get_lessons_in_pair(practice.day, pair)

                    # Проверяем, свободны ли уроки
                    both_free = True
                    for lesson in lessons:
                        if not is_lesson_available(
                                group.id, practice.teacher_id, practice.day, lesson, week, semester,
                                check_main=(fill_type in ['main', 'both']),
                                check_current=(fill_type in ['current', 'both'])
                        ):
                            both_free = False
                            break
                    ...
                    if both_free:
                        ...
                        room = None
                        if practice.room_id:
                            room = Room.query.get(practice.room_id)
                        else:
                            room = find_available_room(practice.day, lessons[0], week, semester)
                        ...
                        if room:
                            ...
                            placed_pairs[(group.id, practice.day, pair)] = practice.subject_id
                            placed = True

        # Затем заполняем обычные предметы
        for group in groups:
            group_subjects = GroupSubject.query.filter_by(group_id=group.id).all()

            # Создаем список предметов с нужным количеством пар
            subjects_to_place = []
            for gs in group_subjects:
                if not gs.teacher_id:
                    log_entry.errors += 1
                    print(
                        f"Ошибка: у предмета {gs.subject.name if gs.subject else 'Неизвестно'} группы {group.name} не назначен преподаватель")
                    continue

                pairs_needed = gs.hours_per_week // 2
                if pairs_needed > 0:
                    subjects_to_place.extend([{
                        'subject_id': gs.subject_id,
                        'teacher_id': gs.teacher_id,
                        'hours_per_week': gs.hours_per_week,
                        'subject_name': gs.subject.name if gs.subject else 'Неизвестно'
            ...
            # Получаем день практики
            practice_day = None
            practice = GroupPractice.query.filter_by(group_id=group.id).first()
            if practice:
                practice_day = practice.day

            # Для каждого предмета ищем свободное место
            for subject_data in subjects_to_place:
                placed = False
                ...
                for day in days_shuffled:
                    ...
                    # Пропускаем день практики
                    if practice_day and day == practice_day:
                        continue

                    max_pairs = 4 if day != "Суббота" else 2

                    # Получаем текущую нагрузку
                    current_load = get_day_load(group.id, day, week, semester)

                    # Проверяем все пары по порядку, чтобы не было дырок
                    for pair in range(1, max_pairs + 1):
                        if placed:
                            break

                        # Проверяем, не занята ли эта пара
                        if (group.id, day, pair) in placed_pairs:
                            continue
                        ...
                        for lesson in lessons:
                            if not is_lesson_available(
                                    group.id, subject_data['teacher_id'], day, lesson, week, semester,
                                    check_main=(fill_type in ['main', 'both']),
                                    check_current=(fill_type in ['current', 'both'])
                            ):
                                both_free = False
                        ...
                        if both_free:
                            # Ищем свободную аудиторию
                            room = find_available_room(day, lessons[0], week, semester)
                            ...
                            if room:
                                ...
                                placed_pairs[(group.id, day, pair)] = subject_data['subject_id']
                                placed = True
        ...
        log_entry.errors += 1
        db.session.commit()
        print(f"Ошибка в автозаполнении: {str(e)}")
        raise e
            ...
            try:
                with db.engine.connect() as conn:
                    ...
                    if 'total_hours_semester1' not in column_names:
                        ...
                        conn.execute(
                            text("ALTER TABLE group_subject ADD COLUMN total_hours_semester1 INTEGER DEFAULT 0"))

                    if 'total_hours_semester2' not in column_names:
                        print("Добавляем колонку total_hours_semester2...")
                        conn.execute(
                            text("ALTER TABLE group_subject ADD COLUMN total_hours_semester2 INTEGER DEFAULT 0"))
